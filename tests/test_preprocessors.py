...
from terrorblade.examples.create_db_from_tg_json import (
...
class TestTelegramWorkflow:
    ...
    def setup_class(cls) -> None:
        ...
        cls.test_data_path = (
            Path(__file__).parent / "data" / "messages_test_animals.json"
        )
    ...
    def _create_small_test_dataset(cls) -> None:
        ...
        for chat in full_data["chats"]["list"]:
            ...
            text_messages = [
                ...
                and len(msg.get("text", "").strip()) > 5
            ]

            if (
                len(text_messages) >= 5
            ):  # Only include chats with enough text messages
    ...
    def text_preprocessor(self) -> TextPreprocessor:
        ...
        return TextPreprocessor(
            time_window="5m", cluster_size=2, batch_size=100
        )
    ...
    def telegram_preprocessor(self) -> TelegramPreprocessor:
        ...
        return TelegramPreprocessor(
            use_duckdb=False, time_window="5m", cluster_size=2
        )
    ...
    def telegram_preprocessor_with_db(self) -> TelegramPreprocessor:
        ...
        db_path = (
            ...
            / f"test_preprocessor_{datetime.now().timestamp()}.db"
        )
        return TelegramPreprocessor(
            use_duckdb=True,
            db_path=str(db_path),
    ...
    def telegram_database(self) -> TelegramDatabase:
        ...
        db_path = (
            self.__class__.temp_dir / f"test_db_{datetime.now().timestamp()}.db"
        )
    ...
    def test_textpreprocessor_init(
    ...
    def test_concat_author_messages(
        ...
        assert (
            "Hello world!. How are you doing today?" in result["text"].to_list()
        )
        expected_columns = [
            "chat_name",
            "date",
    ...
    def test_calculate_embeddings(
        ...
        )  # Should have tensor-like properties
    ...
    def test_calculate_distances(
        ...
        assert torch.all(
            distances >= -1e-6
        )  # Distances should be non-negative (with tolerance)
    ...
    def test_calculate_sliding_distances(
        self, text_preprocessor: TextPreprocessor
    ) -> None:
        """Test sliding window distance calculation."""
        embeddings = torch.randn(5, 10)
        distances = text_preprocessor.calculate_sliding_distances(
            embeddings, window_size=2
        )
    ...
    def test_process_message_groups(
        ...
        result = text_preprocessor.process_message_groups(
            sample_messages_df, time_window="5m", cluster_size=2
        )
    ...
    def test_process_message_groups_empty_input(
        self, text_preprocessor: TextPreprocessor
    ) -> None:
    ...
    def test_telegram_preprocessor_init(
    ...
    def test_load_json_valid_file(
        ...
        result = telegram_preprocessor.load_json(
            str(self.__class__.small_test_file)
        )
    ...
    def test_parse_members(
        ...
        assert (
            len(non_none_results) >= 2
        )  # At least the first two should be processed
    ...
    def test_delete_service_messages(
        self, telegram_preprocessor: TelegramPreprocessor
    ) -> None:
        ...
        assert all(
            chat_type != "service"
            for chat_type in result["chat_type"].to_list()
        )
    ...
    def test_prepare_data(
        ...
        result = telegram_preprocessor.prepare_data(
            str(self.__class__.small_test_file)
        )
    ...
    def test_telegram_database_init(
        self, telegram_database: TelegramDatabase
    ) -> None:
    ...
    def test_telegram_database_init_user_tables(
        self, telegram_database: TelegramDatabase
    ) -> None:
    ...
    def test_telegram_database_add_messages(
        ...
        result = telegram_database.db.execute(
    ...
    def test_process_file_with_database(
        ...
        try:
            ...
            result = telegram_preprocessor_with_db.process_file(
                str(self.__class__.small_test_file)
            )
...
def test_parameter_combinations(time_window: str, cluster_size: int) -> None:
    ...
    preprocessor = TextPreprocessor(
        time_window=time_window, cluster_size=cluster_size
    )
