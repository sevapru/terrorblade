...
class UserStats:
    ...
    )  # (chat_id, chat_name, message_count)
    largest_cluster: (
        tuple[int, str, int] | None
    )  # (chat_id, chat_name, cluster_size)
...
class TelegramDatabase:
        ...
        try:
            ...
            media_cols = ", ".join(
                [
                    ...
                    for field, info in MEDIA_TYPES_SCHEMA.items()
                ]
            )
            self.db.execute(f"""
            ...
            fwd_cols = ", ".join(
                [
                    f'"{field}" {info["db_type"]}'
                    for field, info in FORWARDED_SOURCES_SCHEMA.items()
                ]
            )
            self.db.execute(f"""
    ...
    def _ensure_user_exists(self, phone: str) -> bool:
        ...
        try:
            ...
            if (
                ...
                self.logger.warning(
                    f"Required tables for user {phone} do not exist"
                )
                return False
    ...
    def get_all_users(self) -> list[str]:
        ...
        try:
            ...
            result = self.db.execute(
                "SELECT phone FROM users ORDER BY phone"
            ).fetchall()
    ...
    def get_user_stats(self, phone: str) -> UserStats | None:
        ...
        try:
            ...
            total_messages = self.db.execute(f"""
            ...
            largest_chat_tuple = (
                (largest_chat[0], largest_chat[1], largest_chat[2])
                if largest_chat
                else None
            )

            largest_cluster = self.db.execute(f"""
                {latest_names_cte}
                SELECT
                    m.chat_id,
                    ln.chat_name,
            ...
            for chat_id, chat_name, message_count in chat_stats:
                ...
                chat_stats_dict[chat_id] = ChatStats(
                    ...
                    largest_cluster_size=(
                        cluster_stats[2] if cluster_stats else 0
                    ),
                )
    ...
    def get_chat_stats(self, phone: str, chat_id: int) -> ChatStats | None:
        ...
        try:
            ...
            if not chat_info:
                self.logger.warning(
                    f"Chat {chat_id} not found for user {phone}"
                )
                return None
    ...
    def init_user_tables(self, phone: str) -> None:
        ...
        try:
            ...
            # Clusters table
            self.db.execute(
                self.__get_create_clusters_table_sql(clusters_table)
            )
    ...
    def _upsert_media_types(self, names: list[str]) -> dict[str, int]:
        ...
        # Assign new ids
        new_names = [
            n for n in set(names) if n is not None and n not in name_to_id
        ]
        if new_names:
            # Compute next id
            ...
            to_insert = [
                (next_id + i, n) for i, n in enumerate(sorted(new_names))
            ]
            self.db.executemany(
                "INSERT OR IGNORE INTO media_types(media_type_id, name) VALUES (?, ?)",
                to_insert,
            )
    ...
    def _upsert_forwarded_sources(self, names: list[str]) -> dict[str, int]:
        ...
        if not names:
            return {}
        rows = self.db.execute(
            "SELECT forwarded_from_id, name FROM forwarded_sources"
        ).fetchall()
    ...
    def _convert_string_column_to_mapping(
        ...
        if not mapping:
            return df.with_columns(
                pl.lit(None).cast(pl.Int32).alias(target_column)
            )
    ...
    def _upsert_name_mappings(
        self, phone: str, messages_df: pl.DataFrame
    ) -> None:
        ...
        if (
            "chat_id" in messages_df.columns
            and "chat_name" in messages_df.columns
        ):
            chat_pairs = messages_df.select(
                ["chat_id", "chat_name", "date"]
            ).drop_nulls(subset=["chat_id", "chat_name"])
            if chat_pairs.height > 0:
                ...
                for _row in dupes:
                    self.logger.warning(
                        "Chat {_row[0]} has {_row[1]} distinct names"
                    )

        # User names: from_id, from_name
        if (
            "from_id" in messages_df.columns
            and "from_name" in messages_df.columns
        ):
            user_pairs = messages_df.select(
                ["from_id", "from_name", "date"]
            ).drop_nulls(subset=["from_id", "from_name"])
        ...
        if {"message_id", "chat_id", "file_name"}.issubset(
            set(messages_df.columns)
        ):
            files_df = messages_df.select(
                ["message_id", "chat_id", "file_name"]
            ).drop_nulls(subset=["message_id", "chat_id", "file_name"])
    ...
    def add_messages(self, phone: str, messages_df: pl.DataFrame) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Adding {messages_df.height} messages for user {phone}"
            )
            ...
            if "media_type" in messages_df.columns:
                media_names = [
                    x
                    for x in messages_df.select("media_type")
                    .to_series()
                    .drop_nulls()
                    .to_list()
                    if isinstance(x, str)
                ...
                messages_df = self._convert_string_column_to_mapping(
                    messages_df, "media_type", "media_type", mapping
                )
            ...
            if "forwarded_from" in messages_df.columns:
                fwd_names = [
                    x
                    for x in messages_df.select("forwarded_from")
                    .to_series()
                    .drop_nulls()
                    .to_list()
                    if isinstance(x, str)
            ...
            for c in missing:
                messages_df = messages_df.with_columns(
                    pl.lit(None)
                    .cast(TELEGRAM_SCHEMA[c]["polars_type"])
                    .alias(c)
                )
    ...
    def get_largest_cluster_messages(
        self, phone: str
    ) -> pl.DataFrame | pl.Series | None:
        """
        ...
        try:
            ...
            self.logger.error(
                f"Error getting largest cluster messages: {str(e)}"
            )
            raise
        ...
        if user_stats := self.get_user_stats(phone):
            ...
            if user_stats and user_stats.chat_stats:
                ...
                for i, chat in enumerate(top_3_chats, 1):
                    self.logger.info(
                        f"  {i}. {chat.chat_name} ({chat.message_count} messages)"
                    )

            if (
                user_stats.largest_cluster
                and user_stats.largest_cluster[0] is not None
            ):
                self.logger.info(
            ...
            if (
                ...
                and (
                    chat_stats := self.get_chat_stats(
                        phone, user_stats.largest_chat[0]
                    )
                )
            ):
                self.logger.info(
                ...
                if chat_stats.avg_cluster_size is not None:
                    self.logger.info(
                        f"Average cluster size: {chat_stats.avg_cluster_size:.2f}"
                    )
                if chat_stats.largest_cluster_size is not None:
                    self.logger.info(
                        f"Largest cluster size: {chat_stats.largest_cluster_size}"
                    )
            return user_stats
        else:
    ...
    def get_max_message_id(self, phone: str, chat_id: int) -> int:
        ...
        try:
            ...
            self.logger.info(
                f"No messages found for chat {chat_id}, user {phone}"
            )
            return -1
        except Exception as e:
            self.logger.error(
