...
for file_path in input_folder.iterdir():
    if file_path.name.startswith("messages") and file_path.name.endswith(
        ".html"
    ):
        with open(file_path, encoding="utf-8") as file:
            ...
            for message in messages:
                date_time = (
                    message.find("div", class_="pull_right date details")[
                        "title"
                    ]
                ...
                if message.find("div", class_="text"):
                    ...
                    if media.find("a", class_="video_file_wrap"):
                        ...
                        text = media.find("a", class_="media_voice_message")[
                            "href"
                        ]
                        message_type = "voice_message"
                    elif media.find("a", class_="media_audio_file"):
                        text = media.find("a", class_="media_audio_file")[
                            "href"
                        ]
                        message_type = "audio"
