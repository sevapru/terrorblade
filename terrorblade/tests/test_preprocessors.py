...
def sample_telegram_data():
    return {
        ...
        "members": [["user1", "user2"], ["user2", "user3"], ["user1", "user3"]],
        "reactions": [[{"emoji": "👍"}], None, [{"emoji": "❤️"}]],
    }
...
def test_parse_members(telegram_preprocessor):
    df = pl.DataFrame({"members": [["user1", "user2"], ["user2", "user3"], ["user1", "user3"]]})
...
def test_parse_reactions(telegram_preprocessor):
    df = pl.DataFrame({"reactions": [[{"emoji": "👍"}], None, [{"emoji": "❤️"}]]})
