...
from thoth.utils import Config , NICE
...
class ThothAnalyzer:
    ...
    def import_from_duckdb(self) -> None:
        ...
        # Get all chats
        chat_table = f"chats_{self.phone}" if self.phone else "chats"
        chat_query = f"SELECT id, name FROM {chat_table} ORDER BY id"
        ...
        try:
            ...
            for chat_id, chat_name in chats:
                ...
                self.logger.nice(
                    ...
                    chat_name, chat_id, len(messages), chat_processing_time, 
                    len(messages) / chat_processing_time if chat_processing_time > 0 else 0
                )
            
            # Log completion metrics
            total_time = time.time() - start_time
            total_messages = sum(len(con.execute(
                f"SELECT id FROM {msg_table} WHERE chat_id = {chat_id} AND text != ''").fetchall()
            ) for chat_id, _ in chats)
            
            self.logger.nice(
                "Completed import of %d chats with %d messages in %.2f seconds (%.1f msgs/sec)",
                len(chats), total_messages, total_time, 
    ...
    def analyze_user_topics(self, min_messages: int = 10) -> Dict[str, List[int]]:
        ...
        return {user: topic_list for user, topic_list in user_topics.items() 
    ...
    def analyze_topic_sentiment(self, topic_id: Optional[int] = None) -> Dict[int, float]:
        ...
        for i, cluster_id in enumerate(clusters):
            ...
            sentiment = np.dot(vectors[i], sentiment_vector) / (np.linalg.norm(vectors[i]) * np.linalg.norm(sentiment_vector))
    ...
    def analyze_user_interactions(self, min_interactions: int = 5) -> List[Dict[str, Any]]:
        ...
        significant_edges = [(u, v, d) for u, v, d in user_graph.edges(data=True) 
    ...
    def predict_topic_trends(self, time_window: str = "1 day", prediction_days: int = 7) -> Dict[int, List[Tuple[datetime, float]]]:
    ...
    def analyze_cross_chat_topics(self, min_similarity: float = 0.7) -> List[Dict[str, Any]]:
        ...
        for cluster_id in range(n_clusters):
            ...
            if len(chat_counts) >= 2:
                ...
                topic_texts = [points[i].payload.get("text", "") 
    ...
    def analyze_topic_lifecycle(self) -> Dict[int, Dict[str, Any]]:
        ...
        for topic_id in range(n_clusters):
            # Get topic keywords
            topic_texts = [date_point_map[i][1].payload.get("text", "") 
                         for i in range(len(date_point_map)) if clusters[i] == topic_id]
            ...
            topic_daily_data = [{
                ...
                "percentage": day["topic_counts"][topic_id] / day["total_messages"] if day["total_messages"] > 0 else 0
    ...
    def compare_chats(self) -> List[Dict[str, Any]]:
        ...
        # Extract chat IDs
        chat_ids = list(set(point.payload.get("chat_id") for point in points if point.payload.get("chat_id") is not None))
        ...
        for chat_id in chat_ids:
            ...
            # Get active users
            user_counts = Counter([points[i].payload.get("from_id") for i in chat_indices 
        ...
        for i, chat1 in enumerate(chat_ids):
            ...
            for j in range(i + 1, len(chat_ids)):
                ...
                topic_sim = self.calculate_chat_similarity(
                    analysis1["topic_distribution"], 
                    analysis2["topic_distribution"]
                )
                
                # Calculate hourly pattern similarity
                pattern_sim = self.calculate_pattern_similarity(
                    analysis1["hourly_pattern"], 
                    analysis2["hourly_pattern"]
                )
    ...
    def analyze_topics(self, 
        ...
        if date_from or date_to:
            ...
            filter_obj = models.Filter(
                must=[
                    models.FieldCondition(
                        key="date",
                        range=models.Range(**date_filter)
                    )
                ]
            )
        ...
        self.logger.nice(
            ...
            len(topics), len(points), total_time
        )
        
        for i, topic in enumerate(topics):
            terms_str = ", ".join(topic["terms"][:5])  # Show first 5 terms
            self.logger.nice(
                "Topic %d: %s (%d messages)", 
                i+1, terms_str, topic["message_count"]
            )
