...
def main() -> None:
    ...
    parser.add_argument("--qdrant-path", type=str, default="./qdrant_db", 
    ...
    topics_parser.add_argument("--action", choices=["common", "evolution", "related", "keywords",
                                                   "sentiment", "activity", "influence", "lifecycle"],
    ...
    users_parser.add_argument("--action", choices=["topics", "interactions", "engagement", "behavior"],
    ...
    cross_parser.add_argument("--action", choices=["topics", "correlations", "compare"],
        ...
        if args.command == "import":
            ...
            if args.action == "topics":
                ...
                for i, interaction in enumerate(results):
                    print(f"{i+1}. {interaction['from_user']} -> {interaction['to_user']}: {interaction['interaction_count']} interactions")
                ...
                for user_id, metrics in results.items():
                    ...
                    if 'topics' in metrics:
                        ...
                        for topic, data in metrics['topics'].items():
                            print(f"    {topic}: {data['message_count']} messages, {data['reply_ratio']:.3f} reply ratio")
            ...
            if args.action == "topics":
                ...
                for i, topic in enumerate(results):
                    ...
                    print(f"  Present in {len(topic['chat_distribution'])} chats with {topic['total_messages']} total messages")
