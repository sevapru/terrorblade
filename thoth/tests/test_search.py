...
class TestSearch:
    ...
    def test_semantic_search(self, mock_analyzer, mock_search_results):
        ...
        # Run the search
        results = mock_analyzer.semantic_search(
            query="test query",
            chat_id=123,
            limit=10
        )
    ...
    def test_semantic_search_all_chats(self, mock_analyzer, mock_search_results):
        ...
        mock_analyzer.qdrant.list_collections.return_value = MagicMock(
            collections=[
                MagicMock(name='chat_123'),
                MagicMock(name='chat_456')
            ]
        )
    ...
    def test_find_messages_by_keywords(self, mock_analyzer, mock_search_results):
        ...
        # Run the method
        results = mock_analyzer.find_messages_by_keywords(
    ...
    def test_find_similar_messages(self, mock_analyzer, mock_search_results):
        ...
        mock_analyzer._get_message_by_id = MagicMock(return_value={
            'message_id': 100,
            'chat_id': 123,
        ...
        # Run the method
        results = mock_analyzer.find_similar_messages(
            chat_id=123,
            message_id=100,
            limit=5
        )
    ...
    def test_find_chat_context(self, mock_analyzer, mock_search_results):
        ...
        # Run the method
        results = mock_analyzer.find_chat_context(
            chat_id=123,
            message_id=2,
            context_messages=1
        )
